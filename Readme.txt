...
The following project provides an implementation of our learners used for the project 2 report

_____________________________________________________
RUNNING THE CODE:
The code should be executable from top to bottom, reproducing the results submitted in our report.

The csv files submitted to Kaggle are made using create_csv.

Instead of commenting every single line, we have chosen to organise the code using appropriate markdown throughout, and variable 
names that reflect what is happening in the code.

_____________________________________________________
MODEL RESULTS:
Model results that are submitted for the report are found in the classification reports printed out after running the models.

This is so we could appropriately compare and present results across models.

CSV results can also be found with the submission. As mentioned, these were created using the create_csv function towards the end of the code

_____________________________________________________
create_csv FUNCTION:
The function def create_csv is used to generate csv files to be submitted to Kaggle.
It can be used to reproduce the csv files with predictions using appropriate parameters.

To use this function, pass in the predictions on the test set, as well as the name. 
Predictions should be appropriately named based on what they are (e.g. what models is
used, or how is the data manipualted).

Some of the csv files created will be not found in the code, as these were from changing
hyperparameters or experimenting with models that have not made it into our final submission.
They are still included in the evaluation 


_____________________________________________________
SPLITTING CHOICE MADE:

Because we want to evaluate on more metrics than just the accuracy score given on kaggle,
we decided to split the data to produce more classification reports that show recall
precision and F1 scores too.

This was we can get more insight into the model performance and see 
...
Fiinally -the original readme.txt file containing information about the dataset can be found below:
_____________________________________________________
